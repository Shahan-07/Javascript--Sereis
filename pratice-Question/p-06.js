...
},
{
     ...
     */
        ...
        const findTags = accounts[0].tags.flat().includes('consectetur');
     ...
     */
        ...
        const joining = accounts.map(({tags}) => tags.join(',')).join(',');
       // console.log(joining);
     ...
     */
        ...
        const friend = accounts.map(({friends}) => friends.map(({name}) => name)).flat();
        //console.log(friend);
     ...
     */
            ...
            const friendAccount = accounts.filter(({balance,age}) => age <=30 && parseInt(balance.replace(/\D/g,'')) >1500).flat();
