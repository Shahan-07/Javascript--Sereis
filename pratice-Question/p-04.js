...
const userId = users.find(user => user.firstName === 'Madison' && user.lastName === 'Marshall');
   //   console.log(userId);
...
// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
    ...
    const comm = users.find(user => user.id === comments[0].userId);
        //console.log(comm);
...
const commen = comments.find(Comment => Comment.text === 'OK great thanks');
    // console.log(commen);
...
const commYes = users.filter(user => comments.find(Comment => Comment.userId === user.id));
// console.log(commYes);

const commNot = users.filter(user =>!comments.find(Comment => Comment.userId === user.id));
console.log(commNot);
